...
#    logUtil.apaga_log_transfer()
    ...
    if (lerConfig.get_testes('uploadDirRecursivo')=='True'):
        ...
        for usuario in lusuario:
            ...
            util.recriaDiretorioBase(oc00)
            for i in range(qtd):
                thread.start_new( util.enviaDiretorioRecursivo, (oc00,folder,'Th0'+str(i)+'-'+socket.gethostname()) )
    ...
    if (lerConfig.get_testes('uploadDirPeriodo')=='True'):
        ...
        for usuario in lusuario:
            ...
            for i in range(nthead):
                thread.start_new( util.cicloPeriodo, (oc00,'dir00T'+str(nthead)+'-'+str(i)+'-'+socket.gethostname(),tempo, nthead))
